...
import { server } from "../..";
import PurchasedItemsHistory from "./PurchasedItemsHistory";

function KhataCutomers({ userId }) {
  ...
  useEffect(() => {
    // setInvoiceId(uuidv4())
    // const uuid = uuidv4();
    const min = 100000000000; // Minimum 7-digit number
    const max = 999999999999; // Maximum 7-digit number
    const randomNumber = Math.floor(Math.random() * (max - min + 1)) + min;
    const nextInvoiceId = `INVKTADSTNR${randomNumber}`;
    //set invoice id
